...
Here we plot the ratio of male to female characters over time to get a better idea of how they compare. The ratio is always above 1, even today, as marked by the red dashed line, meaning more males than females are introduced every year.

At first the graph oscilates heavily due to limited female characters; there are many more males vs. females introduced, creating large proportions. However, some years there are many more males indroduced due to more characters introduced in general, and some years it's a little closer.

As time goes on, it steadily declines and smooths out over time, indicating ratios closer to 1 and female character introductions.
...
gender_counts_wide <- gender_counts %>%
  ...
  mutate(
    ratio = ifelse(Female == 0, NA, Male / Female)
  )

gender_counts_wide %>% 
  arrange(desc(ratio)) %>%
  select(year, Male, Female, ratio)

gf_line(ratio ~ year, data = gender_counts_wide, color = "blue") %>%
  gf_hline(yintercept = 1, color = "red", linetype = "dashed") %>%
  gf_labs(
...
Filter for characters that have valid alignments and have appeared at least once in a comic. Since there are only 3 reformed characters we ignore those and focus on Good vs. Bad characters. We also do drop 451 rows for good and 539 for bad; however, there are still just over 7,000 characters with appearances for good and 9,076 characters with appearances for bad, so we keep a sizable amount of the data to explore further.
...
comic_clean %>%
  ...
  summarize(
    total = n(),
    appearances_missing = sum(is.na(appearances)),
    appearances_present = sum(!is.na(appearances))
  )
```
...
## Plotting Appearances Distribution by Alignment

The following plots show how Bad characters are more concentrated in their number of appearances while Good characters have far high standard deviation. Bad characters have a median of 3 but a mean of 9 while Good characters have a median of 6 and a mean of 35.
...
appearance_data %>%
  ...
  summarise(
    n = n(),
    mean_appearances = mean(appearances),
    median_appearances = median(appearances),
    sd_appearances = sd(appearances)
  )

gf_boxplot(appearances ~ align, data = appearance_data) %>%
  gf_labs(
    title = "Comic Appearances by Character Alignment",
    x = "Alignment",
...
The following ANOVA table revealed a highly significant effect of alignment on the number of comic appearances (F about = 287 and p \< 2e-16).

This means Good and Bad characters do not share the same average number of appearances. The differences observed between alignments are far too large to be explained by random chance, indicating that moral alignment is strongly associated with how frequently a character appears in comics and we reject the null hypothesis.
...
Just like when testing alignment, we filter out reformed and make sure we're only working with characters with alive status data.

```{r}
alive_data <- comic_clean %>%
  filter(!is.na(align),
         align %in% c("Good", "Bad"),
         !is.na(alive))

alive_summary <- alive_data %>%
  group_by(align, alive) %>%
  summarise(n = n()) %>%
  mutate(prop = n / sum(n))
```

The following plot reveals Good characters have the highest average appearances, but the median is low. Thus indicates a few characters that dominate number of appearances.
...
```{r}
gf_bar(~align, fill = ~alive, data = alive_data, position = "fill") %>%
  gf_labs(
